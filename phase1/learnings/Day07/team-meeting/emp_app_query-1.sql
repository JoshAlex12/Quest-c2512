...
select id,
       name
  from departments; // best practice 
...
-- Display deparments names for ids between 10 and 30 
select name
  from departments
...
-- Display department by name = 'Alpha' [athira]
select id,
       name
  from departments
 where name = 'Alpha';
...
select id,
 ...
 where ( country_id = 11 );
    ...
    -- select country id for given country name = "India" 
select id
  from countries
 where name = 'India';
...
select 64 * ( 60 / 2 ) as total_projects
  from dual;
...
select id,
 ...
 where ( city_id in (
   select id
    ...
    -- find the department id and the number of employees in each respective department
